...
time <- system.time({
  ...
  # for array in {1..379};
  # do
  # dx run app-swiss-army-knife -iin=UKB_PRS:JW/Software/r_with_plink.tar.gz -iin=UKB_PRS:MetaSTAAR/Scripts/Meta_Coding_470.R -iin=UKB_PRS:MetaSTAAR/Scripts/Meta_Coding_470.sh -icmd="bash Meta_Coding_470.sh ${array}" -y --destination UKB_PRS:MetaSTAAR/Results_470/ --priority low --extra-args '{"executionPolicy":{"maxSpotTries":5,"spotOnly":true}}' --instance-type mem3_ssd1_v2_x8
  # done
  ...
  save(coding_sumstat,file=paste0(output_file_name,"_sumstat_",arrayid,"_470.Rdata"),compress = "xz")
  save(coding_cov,file=paste0(output_file_name,"_cov_",arrayid,"_470.Rdata"),compress = "xz")
  
  seqClose(genofile)
  
  system("rm obj.STAAR.UKB.TC.20240707.Rdata")
  system(paste0("rm ",agds.path))
  system("rm Annotation_name_catalog.csv")
})[3]

save(time,file = paste0(output_file_name,"_time_",arrayid,"_470.Rdata"))