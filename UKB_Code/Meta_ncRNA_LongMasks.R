...
time <- system.time({
  ...
  # dx run app-swiss-army-knife -iin=UKB_PRS:JW/Software/r_with_plink.tar.gz -iin=UKB_PRS:MetaSTAAR/Scripts/Meta_ncRNA_LongMasks.R -iin=UKB_PRS:MetaSTAAR/Scripts/Meta_ncRNA_LongMasks.sh -icmd="bash Meta_ncRNA_LongMasks.sh" -y --destination UKB_PRS:MetaSTAAR/Results/ --priority low --extra-args '{"executionPolicy":{"maxSpotTries":5,"spotOnly":true}}' --instance-type mem3_ssd1_v2_x16
  ...
  ###########################################################
  ## Null model
  obj_nullmodel <- get(load("obj.STAAR.UKB.TC.20240226.Rdata"))
  ...
  save(ncRNA_sumstat,file=paste0(output_file_name,"_sumstat_",223,".Rdata"),compress = "xz")
  save(ncRNA_cov,file=paste0(output_file_name,"_cov_",223,".Rdata"),compress = "xz")
...
save(time,file = paste0(output_file_name,"_time_",223,".Rdata"))