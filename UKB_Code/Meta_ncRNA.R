...
time <- system.time({
  ...
  # for array in {1..222};
  # do
  # dx run app-swiss-army-knife -iin=UKB_PRS:JW/Software/r_with_plink.tar.gz -iin=UKB_PRS:MetaSTAAR/Scripts/Meta_ncRNA.R -iin=UKB_PRS:MetaSTAAR/Scripts/Meta_ncRNA.sh -icmd="bash Meta_ncRNA.sh ${array}" -y --destination UKB_PRS:MetaSTAAR/Results/ --priority low --extra-args '{"executionPolicy":{"maxSpotTries":5,"spotOnly":true}}' --instance-type mem3_ssd1_v2_x4
  # done
  ...
  save(ncRNA_sumstat,file=paste0(output_file_name,"_sumstat_",arrayid,".Rdata"),compress = "xz")
  save(ncRNA_cov,file=paste0(output_file_name,"_cov_",arrayid,".Rdata"),compress = "xz")
