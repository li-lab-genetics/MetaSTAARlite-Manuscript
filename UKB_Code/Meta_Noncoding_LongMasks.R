...
time <- system.time({
  ...
  # for array in {1..8};
  # do
  # dx run app-swiss-army-knife -iin=UKB_PRS:JW/Software/r_with_plink.tar.gz -iin=UKB_PRS:MetaSTAAR/Scripts/Meta_Noncoding_LongMasks.R -iin=UKB_PRS:MetaSTAAR/Scripts/Meta_Noncoding_LongMasks.sh -icmd="bash Meta_Noncoding_LongMasks.sh ${array}" -y --destination UKB_PRS:MetaSTAAR/Results/ --priority low --extra-args '{"executionPolicy":{"maxSpotTries":5,"spotOnly":true}}' --instance-type mem3_ssd1_v2_x16
  # done
  ...
  save(noncoding_sumstat,file=paste0(output_file_name,"_sumstat_",arrayid_longmask+379,".Rdata"),compress = "xz")
  save(noncoding_cov,file=paste0(output_file_name,"_cov_",arrayid_longmask+379,".Rdata"),compress = "xz")
